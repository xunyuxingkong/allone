...
首批样例位于 `../cases/mvp/`，可通过项目虚拟环境执行：

```bash
XGTEST_DB_HOST=... XGTEST_DB_PORT=1907 XGTEST_DB_NAME=SYSTEM \
XGTEST_DB_USER=... XGTEST_DB_PASSWORD=... \
xgtest run --cases cases/mvp --output artifacts/runs/mvp.json
```

Runner 覆盖 YAML 加载、setup/statement/query/cleanup、预期行比较和脱敏 JSON 报告；真实虚谷环境的连接与清理验证记录在审核意见目录的已完成清单中。
